...
def wc_generator(image,depth,water_type=1,water_depth=0,water_illum=[1,0,0]):
    ...
    L_t3, v= get_art_light(x, y,eta, depth, sigma,x_c=np.random.randint(0,x.shape[1]),y_c=np.random.randint(0,x.shape[0]),L_art=np.random.uniform(0.6, 0.99, 1),Z_l= z_l,r_l=r_l)
    L_t=water_illum[0]*L_t1+water_illum[1]*L_t2+water_illum[2]*L_t3 #L_t3
    ...
    eta_haze = eta_haze.transpose((0, 3, 1, 2))
